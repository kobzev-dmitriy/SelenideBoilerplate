...
public class UsersScenarTests extends A_BaseTest {
    ...
    @Test
    // 3909 Пользователи. Деактивация и активация созданного, попытка авторизации
    ...
    /*@Test
    // !!!FIX IT Предустановленный сейчас используется как основной пользователь
    ...
    */
    ...
    public void I_formModuleUsers() {
        app.mainPage.openUsers();
        Driver.refresh();
        Assert.assertTrue(app.listUsersPage.countRowResult() > 0);
    ...
    public void J_searchByName() {
        ...
        app.mainPage.openUsers();
        int countEmptySearch = app.listUsersPage.countRowResult();

        app.listUsersPage.searchUserByName("TestSearch");
        Assert.assertEquals(app.listUsersPage.countRowResult(), 2);
        app.listUsersPage.searchUserByName("TestSearch1");
        Assert.assertEquals(app.listUsersPage.countRowResult(), 1);
        app.listUsersPage.searchUserByName("TestSearch2");
        Assert.assertEquals(app.listUsersPage.countRowResult(), 1);

        app.listUsersPage.searchUserByName("");
        Assert.assertEquals(app.listUsersPage.countRowResult(), countEmptySearch);

        app.listUsersPage.searchUserByName("RD6idrfwsNgfdwkfwj");
        app.listUsersPage.emptySearchResultMsgEnabled();
        Assert.assertEquals(app.listUsersPage.countRowResult(), 0);
    ...
    public void M_sortUsers() {
        ...
        UsersData lastUserData = new UsersData()
                ...
                .withEmail("zzzzzzzzz@email.com").withPassword("Deep2000").withDescription("яяяяяяяя");
        app.mainPage.openUsers();
        app.listUsersPage.createButtonClick();
    ...
    // 3961 Пользователи. Удаление 25+
    public void Q_createLoginUser25plus() {
        // Создаем 27 пользователей (например admin1 - admin27)
        app.mainPage.openUsers();
        //int countRowsFirstPage = app.listUsersPage.countRowResult();
        int countRowsFirstPage = 1;
        int countUsers = 27;
        //for(int i = 1; i <= countUsers; i++) {
            ///UsersData userData = new UsersData()
                    //.withLogin("admin" + i).withName("admin" + i).withSurname("admin" + i).withFathername("admin" + i)
                    //.withPhone("Keys.NUMPAD7, Keys.NUMPAD9, Keys.NUMPAD1, Keys.NUMPAD2, Keys.NUMPAD3, Keys.NUMPAD4,Keys.NUMPAD5,Keys.NUMPAD6,Keys.NUMPAD7,Keys.NUMPAD8,Keys.NUMPAD9")
                    //.withEmail("admin" + i + "@email.com").withPassword("Password").withDescription("admin" + i);
            //app.mainPage.openUsers();
            //app.listUsersPage.createButtonClick();
            //app.createUsersPage.createUser(userData);
            //app.createUsersPage.successMessageClick();
        ...
        app.mainPage.openUsers();
        Assert.assertEquals(app.listUsersPage.countRowResult(), 25);
        app.listUsersPage.nextPagePaginationClick();
        int countAllRowsSecondPage = countRowsFirstPage + (countUsers - 25);
        Assert.assertEquals(app.listUsersPage.countRowResult(), countAllRowsSecondPage);

        UsersData lastUserData = new UsersData()
                .withLogin("admin" + countUsers).withName("admin" + countUsers).withSurname("admin" + countUsers).withFathername("admin" + countUsers)
                .withPhone("Keys.NUMPAD7, Keys.NUMPAD9, Keys.NUMPAD1, Keys.NUMPAD2, Keys.NUMPAD3, Keys.NUMPAD4,Keys.NUMPAD5,Keys.NUMPAD6,Keys.NUMPAD7,Keys.NUMPAD8,Keys.NUMPAD9")
                .withEmail("admin" + countUsers + "@email.com").withPassword("Password").withDescription("admin" + countUsers).withGroup("Администратор");
        Assert.assertEquals(app.listUsersPage.getLoginUserListByIndex(countAllRowsSecondPage), lastUserData.getLogin());
        Assert.assertEquals(app.listUsersPage.getNameUserListByIndex(countAllRowsSecondPage), lastUserData.getName());
        Assert.assertEquals(app.listUsersPage.getSurnameUserListByIndex(countAllRowsSecondPage), lastUserData.getSurname());

        UsersData prevUserData = new UsersData()
                .withLogin("admin" + (countUsers - 1)).withName("admin" + (countUsers - 1)).withSurname("admin" + (countUsers - 1)).withFathername("admin" + (countUsers - 1))
                .withPhone("Keys.NUMPAD7, Keys.NUMPAD9, Keys.NUMPAD1, Keys.NUMPAD2, Keys.NUMPAD3, Keys.NUMPAD4,Keys.NUMPAD5,Keys.NUMPAD6,Keys.NUMPAD7,Keys.NUMPAD8,Keys.NUMPAD9")
                .withEmail("admin" + (countUsers - 1) + "@email.com").withPassword("Password").withDescription("admin" + (countUsers - 1)).withGroup("Администратор");
        Assert.assertEquals(app.listUsersPage.getLoginUserListByIndex(countAllRowsSecondPage - 1), prevUserData.getLogin());
        Assert.assertEquals(app.listUsersPage.getNameUserListByIndex(countAllRowsSecondPage - 1), prevUserData.getName());
        Assert.assertEquals(app.listUsersPage.getSurnameUserListByIndex(countAllRowsSecondPage - 1), prevUserData.getSurname());
        app.listUsersPage.clickEditListUsersByIndex(countAllRowsSecondPage);

        app.mainPage.openUsers();
        app.mainPage.logout();

        app.loginPage.open();
        app.loginPage.login("admin" + countUsers, "Password");
        app.mainPage.openUsers();
        app.mainPage.logout();
        ...
        // Изменение 25+. Редактировать 27го
        app.mainPage.openUsers();
        app.listUsersPage.nextPagePaginationClick();
        app.listUsersPage.clickEditListUsersByIndex(countAllRowsSecondPage);
        // Изменить значения у всех полей
        UsersData userData = new UsersData()
                .withLogin("TestEdit").withName("TestEdit").withSurname("TestEdit").withFathername("TestEdit")
                .withPhone("Keys.NUMPAD7, Keys.NUMPAD9, Keys.NUMPAD8, Keys.NUMPAD7, Keys.NUMPAD6, Keys.NUMPAD5,Keys.NUMPAD4,Keys.NUMPAD3,Keys.NUMPAD2,Keys.NUMPAD1,Keys.NUMPAD0")
                .withEmail("TestEdit@email.com").withPassword("Deep2000").withDescription("TestEdit").withGroup("Администратор");
        app.createUsersPage.editUser(userData);
        app.createUsersPage.saveButtonClick();
        app.createUsersPage.successMessageClick();
        app.mainPage.openUsers();
        ...
        // Проверяем корректность изменений значений полей в списке пользователей
        app.mainPage.openUsers();
        Assert.assertEquals(app.listUsersPage.getLoginUserListByIndex(countAllRowsSecondPage), userData.getLogin());
        Assert.assertEquals(app.listUsersPage.getNameUserListByIndex(countAllRowsSecondPage), userData.getName());
        Assert.assertEquals(app.listUsersPage.getSurnameUserListByIndex(countAllRowsSecondPage), userData.getSurname());
        Assert.assertEquals(app.listUsersPage.getPhoneUserListByIndex(countAllRowsSecondPage), "+79876543210");
        Assert.assertEquals(app.listUsersPage.getEmailUserListByIndex(countAllRowsSecondPage), userData.getEmail());
        Assert.assertEquals(app.listUsersPage.getActivityUserListByIndex(countAllRowsSecondPage), "Да");
        // Проверяем корректность изменений значений полей в форме редактирования пользователей
        app.listUsersPage.clickEditListUsersByIndex(countAllRowsSecondPage);
        Assert.assertEquals(app.createUsersPage.infoFromEditUserForm().getLogin(), userData.getLogin());
        Assert.assertEquals(app.createUsersPage.infoFromEditUserForm().getName(), userData.getName());
        ...
        Assert.assertEquals(app.createUsersPage.infoFromEditUserForm().getFathername(), userData.getFathername());
        Assert.assertEquals(app.createUsersPage.infoFromEditUserForm().getEmail(), userData.getEmail());
        Assert.assertEquals(app.createUsersPage.infoFromEditUserForm().getPhone(), "+79876543210");
        Assert.assertEquals(app.createUsersPage.infoFromEditUserForm().getGroup(), userData.getGroup());
        Assert.assertEquals(app.createUsersPage.infoFromEditUserForm().getDescription(), userData.getDescription());
        app.mainPage.logout();
        ...
        // Попытаться авторизоваться с новыми данными
        app.loginPage.open();
        app.loginPage.login("TestEdit", "Deep2000");
        app.mainPage.openUsers();
        app.mainPage.logout();

        app.loginPage.open();
        app.loginPage.login(AppConfig.loginUser, AppConfig.passwordUser);
        // Возвращаем значения пользователю admin27
        app.loginPage.open();
        app.mainPage.openUsers();
        app.listUsersPage.nextPagePaginationClick();
        app.listUsersPage.clickEditListUsersByIndex(countAllRowsSecondPage);
        app.createUsersPage.editUser(lastUserData);
        app.createUsersPage.saveButtonClick();
        app.createUsersPage.successMessageClick();
        app.mainPage.openUsers();
        app.listUsersPage.nextPagePaginationClick();
        // Нажать Удалить у 26го
        app.listUsersPage.clickDeleteListUsersByIndex(countAllRowsSecondPage);
        // Подтвердить удаление
        app.listUsersPage.acceptDeleteUserClick();
        app.listUsersPage.successMessageListClick();
        // Окно закрылось, пользователь не отображается в списке
        Assert.assertEquals(app.listUsersPage.countRowResult(), countAllRowsSecondPage - 1);
        // Обновить страницу по CTRL+F5. Пользователя нет в списке
        Driver.refresh();
        app.listUsersPage.nextPagePaginationClick();
        Assert.assertEquals(app.listUsersPage.countRowResult(), countAllRowsSecondPage - 1);
        app.mainPage.openUsers();
        ...
        app.createUsersPage.successMessageClick();
        //for(int i = 1; i < countUsers; i++) {
            //app.listUsersPage.deleteUserBySearchName("admin" + countUsers);
        //}
    }
    */
