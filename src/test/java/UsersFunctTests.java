...
public class UsersFunctTests extends A_BaseTest
{
    ...
    public void B_requiredFieldsCreateUser() {
        ...
        logger.info("Нажимаем сразу \"Сохранить\" Не успешно, подсвечены красным обязательные поля (Логин*, Имя*, Фамилия*, Пароль*, Группы*)");
    ...
    public void C_reguiredFieldsEditFormUser() {
        logger.info("Создаем пользователя с именем Name1");
        ...
        app.mainPage.openUsers();
        // Умышленная ошибка - исправить
        //logger.info("Создаем пользователя с именем Name1234");
        app.listUsersPage.searchUserByName("Name1");
        //app.listUsersPage.searchUserByName("Name1");
        // Проверяем корректность значений полей в списке пользователей
        logger.info("Проверяем корректность значений полей в списке пользователей");
